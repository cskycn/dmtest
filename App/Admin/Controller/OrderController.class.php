...
class OrderController extends PublicController{
	...
	public function index(){
		...
		//$activityList=M('activity')->order('id desc')->limit($limit,rows)->select();

		$activityList = M()->table('dm_activity a, dm_user b, dm_test c')
						   ->where('b.id = a.user_id && c.id = a.test_id')
	...
	public function edit(){
		$con["id"] = intval($_REQUEST["activity_id"]);
		$empty = 1;
	...
	public function doEdit(){
		...
		$con["id"] = intval($_POST["aid"]);

		$data["user_name"] = trim($this->clearhtml($_POST["user_name"]));
		$data["phone"] = trim($this->clearhtml($_POST["phone"]));
		$data["room_number"] = trim($this->clearhtml($_POST["room_number"]));
