...
function checkLocation($submit) {
	...
	if(array_key_exists("province",$submit) || array_key_exists("city",$submit) || array_key_exists("area",$submit) || array_key_exists("address",$submit) ){
		if(trim($submit["province"])=='' || trim($submit["city"])=='' || trim($submit["area"])=='' ||
	       trim($submit["province"])=='请选择' || trim($submit["city"])=='请选择' || trim($submit["area"])=='请选择' ||
		   trim($submit["address"])=='') {
			return false;
		}
		else{
...
function clearhtml($content) {  
 ...
 //发送http post请求，并返回结果
 function get_http_array($type,$url,$data){
		 if($type == 'post'){
			$ch = curl_init();
...
//		 $res = object_array($out);
 ...
 //获取截取后的个数
 function getExplodeCount($str='',$sli=','){
	 if(is_string($str)){
		$arr = explode($sli,$str);
		return count($arr);
	 }else{
